...
if __name__=='__main__':
	...
	for k in [1, 5, 10, 25, 50]:
		...
		for train_index, test_index in kf_full:
			...
			#X_train, X_test = mean_imputation_by_type(X_train, X_test, impute)
			#X_train, X_test = regression_imputation(X_train, X_test, impute)
			...
			#Y_random = np.array([np.mean(Y_train)] * Y_test.shape[0])
			
			error = mean_absolute_error(acc, Y_test)
			accs.append(error)
		print k, np.mean(accs)
